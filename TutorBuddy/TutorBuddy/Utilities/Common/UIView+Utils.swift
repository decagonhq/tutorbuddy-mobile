...
extension UIView {
    ...
    func animateViewOnTapGesture(duration: TimeInterval = 0.2, completion: ((_ : String?) -> Void)? = nil) {
        addTapGesture {
            self.animateView(duration: duration, completion: completion)
        }
    }
...
extension UICollectionView {
    ...
    func setEmptyMessage(_ message: String = Constants.DATA_NOT_FOUND) {
        ...
        messageLabel.font = .interRegular(size: 15)
...
extension UISearchBar {
    ...
    convenience init(placeholder: String = "Search", font: UIFont = .interRegular(size: 15), showsCancelButton: Bool = true, searchBarStyle: UISearchBar.Style = .minimal, height: CGFloat? = nil, delegate: UISearchBarDelegate? = nil) {
...
extension UILabel {
    ...
    convenience init(text: String, font: UIFont = .interRegular(size: 16), numberOfLines: Int = 1, color: UIColor = .aLabel, alignment: NSTextAlignment = .center, adjustsFontSizeToFitWidth: Bool = true, underlined: Bool = false, huggingPriority: UILayoutPriority? = nil, huggingAxis: NSLayoutConstraint.Axis? = nil, lineBreakMode: NSLineBreakMode = .byWordWrapping) {
        self.init(frame: .zero)
        self.text = text
